...
class TestSolutionToDot(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # Create folder if it does not exist.
        if not pathlib.Path("out/").exists():
    ...
    def setUp(self):
        ...
        np_state = self.graph.create_np("state")
        np_turn = self.graph.create_np("turn")
        np_node_winner = self.graph.create_np("node_winner")
        np_act = self.graph.create_ep("act")
        np_edge_winner = self.graph.create_ep("edge_winner")
        ...
        for u, v, k in self.graph.edges():
            np_act[u, v, k] = f"a{(u, v, k)}"
            if u % 2 == 0:
                np_edge_winner[u, v, k] = 2
            else:
                np_edge_winner[u, v, k] = 1

        self.fpath_dot = str(pathlib.Path("out/solution.dot").absolute())
        self.fpath_png = str(pathlib.Path("out/solution.png").absolute())
        self.fpath_pdf = str(pathlib.Path("out/solution.pdf").absolute())
        self.fpath_svg = str(pathlib.Path("out/solution.svg").absolute())

    def test_solution_to_dot(self):
        io.to_dot(self.fpath_dot, self.graph, formatting='solution', node_props=['state', "node_winner"], edge_props=['act', "edge_winner"])
        io.dot2png(self.fpath_dot, self.fpath_png)
        io.dot2pdf(self.fpath_dot, self.fpath_pdf)
        io.dot2svg(self.fpath_dot, self.fpath_svg)
