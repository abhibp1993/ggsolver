...
P1 moves freely.
P2 is restricted in 3x3 grid on bottom-left.
...
logger.add("out/ex10.log", format="[{level}]:: {message}", level="DEBUG")
...
class RndGridworld(opac_models.Arena):
    ...
    def states(self):
        ...
        p2_r_max, p2_c_max = self._p2_dim
        return itertools.product(range(r_max), range(c_max), range(p2_r_max), range(p2_c_max), [1, 2])
    ...
    def enabled_acts(self, state):
        ...
        if self.turn(state) == 1:
            ...
            if p1c == 0:
                actions.remove(gw_util.GW_ACT_W)
            if p1r == self._p2_dim[0] - 1:
                actions.remove(gw_util.GW_ACT_N)
            if p1c == self._p2_dim[1] - 1:
                actions.remove(gw_util.GW_ACT_E)
    ...
    def delta(self, state, act):
        ...
        if turn == 1:
            ...
            p2r_prime, p2c_prime = gw_util.bouncy_wall((p2r, p2c), [(p2r_prime, p2c_prime)], self._p2_dim)[0]
